...
namespace EtayfeWeb.Controllers
{
    ...
    public class ProductController : MyBaseController
    {
        ...
        public async Task<IActionResult> GetAllProducts(string search , int? catId, int type, int? price1, int? price2,int? sort ,PostsParameters postsParameters)
        {
            var result = await _productBL.GetAllSearchProducts(catId, GetCurrentUserId(),search, GetLanguage(), sort, type, price1, price2, postsParameters);
            return Ok(result);
        } 
        ...
        {
            var result = await _productBL.GetDelegateProducts(GetCurrentUserId(), GetLanguage(),c => c.ProductCategories.Any(x => x.CategoryId == catId));
            return Ok(result);
        }
