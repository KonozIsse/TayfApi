    ...
    public class CartBL
    {
            ...
            if (code != null)
            {
                ...
                if (coupon != null)
                {
                    ...
                    else if (coupon.DiscountType == "fixed_product")
                    {
                        ...
                        foreach (var item in carts)
                        {
                            var product = await _repositoryManager.Product.GetProductById(item.ProductId, false);
        ...
        public async Task<decimal> AvailableAmountForCart(int cartId, int customerId)
        {
            ...
            if (cartAttributeProducts != null)
            {
                ...
                var attribute = await _repositoryManager.Attribute.GetAttributeIdProductId(productAttribut ,cart.ProductId);
                if (attribute != null)
                {
                    var inventories = await _repositoryManager.Inventory.GetAllInventoryByProductIdOption(cart.ProductId, attribute.Id);
                    foreach (var inventory in inventories)
                    {
                        if (inventory.StockType == "in")
                        {
            ...
            {
                var inventories = await _repositoryManager.Inventory.GetAllInventoryByPrductId(cart.ProductId);
                if (inventories != null && inventories.Count() > 0)
                {
                    foreach (var inventory in inventories)
                    {
