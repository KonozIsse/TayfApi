...
namespace ControlPanel.Controllers
{
    ...
    public class TaxRateController : MyBaseController
    {
        ...
        [HttpGet("get")]
        public async Task<IActionResult> GetTaxeRates(string search, [FromQuery] PostsParameters postsParameters)
        {
            var result = await _locationTaxBL.GetTaxeRates(search,GetLanguage(), postsParameters);
        ...
        {
            var result = await _locationTaxBL.AddTaxRate(GetStoreId(), create);
            if (result.Success)
            {
        ...
        {
            var result = await _locationTaxBL.EditTaxRate(GetStoreId(), update);
            if (result.Success)
            {
                return Ok(result.Message);
            }
