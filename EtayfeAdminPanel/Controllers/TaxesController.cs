...
namespace EtayfeAdminPanel.Controllers
{
    ...
    public class TaxesController : MyBaseController
    {
        ...
        [HttpGet("get")]
        public async Task<IActionResult> GetTaxes(string search,string filter, [FromQuery] PostsParameters postsParameters)
        {
            var result = await _locationTaxBL.GetTaxes(search,filter, GetLanguage(), postsParameters);
