...
namespace EtayfeAdminPanel.Controllers
{
    ...
    public class ProductController : MyBaseController
    {
        ...
        public async Task<IActionResult> GetAllTypesProducts()
        {
            var result =  _productBL.GetProductTypes();
            return Ok(result);
        }
        [HttpGet("getValuesOption")]
        public async Task<IActionResult> GetAllTypesProducts(int id)
        {
            var result = await  _productBL.GetAllValuesToOption(id);
            return Ok(result);
        }
        [HttpGet("allCP")]
