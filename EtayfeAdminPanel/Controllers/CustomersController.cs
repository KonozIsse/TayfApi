...
namespace EtayfeAdminPanel.Controllers
{
    ...
    public class CustomersController : MyBaseController
    {
        ...
        [HttpGet("get")]
        public async Task<IActionResult> GetAllCustomers(string search,string filter, [FromQuery] PostsParameters postsParameters)
        {
            var result = await _userBL.GetCustomers(search, filter, postsParameters);
        ...
        {
            var result = await _userBL.GetAllCustomer();
            return Ok(result);
        }
