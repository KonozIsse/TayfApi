...
namespace EtayfeAdminPanel.Controllers
{
    ...
    public class SettingWebController : MyBaseController
    {
        ...
        public async Task<IActionResult> GetSliderWeb(string search,string filter, [FromQuery] PostsParameters postsParameters)
        {
            var result = await _homeBL.GetSliderWeb(search, filter, GetLanguage(), postsParameters);
